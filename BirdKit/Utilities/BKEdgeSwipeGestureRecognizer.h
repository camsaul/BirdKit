...
PROP NSUInteger edgeThreshold; // default is 40. If user is not within [edgeThreshold] of the edge of the screen when they start the swipe, then ignore it.
PROP CGFloat edgeAmount; // default is 0.4. This is the amount of each edge in which to recognize a swipe (e.g., only recognize a swipe from the middle 40% of any edge)
